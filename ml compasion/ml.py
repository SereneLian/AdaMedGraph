...
parser.add_argument('--model', type=str, default='lr', help='while model to use')
...
if n_class == 3:
    ...
    data_type = '_binary' # '_strict', '_full_imputed'
...
def mlp(i,x_train_all= x_train_all, x_train= x_train, x_val=x_val, x_test= x_test):
        ...
        if model_use =='lr':
                ...
                grid = {
                        ...
                        'model__tol':[ 0.0001, 0.00001],
                        'model__random_state':[42, 3047, 21]
                        }
        elif model_use == 'rf':
...
for i in range(len(label_tar)):
    if n_class ==3:
        file_name = 'new_results/'+model_use+'/ppmi/'+task+'/'+label_tar[i]
